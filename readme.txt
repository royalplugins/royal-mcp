...
= 1.4.15 =
Critical update: fixes API key Regenerate button being silently overridden (clicking did nothing pre-1.4.15), switches new keys to lowercase hex to eliminate uppercase/lowercase character ambiguity in monospace admin fonts, and replaces the fixed 1-hour MCP session TTL with a sliding 24-hour window so active Claude Desktop sessions stop dying mid-day. Existing keys keep working — only newly-regenerated keys use the new hex format.
